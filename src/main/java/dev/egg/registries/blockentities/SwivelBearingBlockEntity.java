...
public class SwivelBearingBlockEntity extends BlockEntityAccessor {
    @Override
    public CompoundTag modifyNBT(final CompoundTag tag, final HashMap<UUID, Pair<UUID, Vec3i>> oldToNewSubLevelIDMap, final Pair<Vector3d,Vector3d> translation) {
        ...
        if (tag.hasUUID("SubLevelID")) {
            var newGoal = oldToNewSubLevelIDMap.get(tag.getUUID("SubLevelID"));
            if (newGoal != null) {
                UUID goalSubLevel = newGoal.first;
                Vec3i offset = newGoal.second;
                tag.putUUID("SubLevelID", goalSubLevel);

                if (tag.contains("SwivelPlate")) {
                    var list = tag.getIntArray("SwivelPlate");
                    BlockPos pos = new BlockPos(list[0],list[1],list[2]);
                    pos = pos.offset(offset);
                    tag.putIntArray("SwivelPlate", new int[]{pos.getX(),pos.getY(),pos.getZ()});
                }
            }
        }
